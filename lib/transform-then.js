...
module.exports = function(source, filepath) {
  ...
  function createMetaObjectAsExtraArgument(fnAst, callAst) {
    var evaluators = []
    var originalParams = fnAst.params;
    ...
    function appendMetaObjectToArgumentList() {
      ...
      if (isBinaryExpression) {
        metaAst.properties[2].value = makeEvaluatorFunction(returnedExpression().left, originalParams)
        metaAst.properties[3].value = makeEvaluatorFunction(returnedExpression().right, originalParams)
      }
